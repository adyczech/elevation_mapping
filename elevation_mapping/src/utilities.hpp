...
bool configure_native_thread(T native_handle, ThreadPriority priority, int cpu_id)
{
  ...
  if (priority == ThreadPriority::HIGH) {
    ...
    params.sched_priority =
      (sched_get_priority_min(SCHED_FIFO) + sched_get_priority_max(SCHED_FIFO)) / 2 - 1;
  } else if (priority == ThreadPriority::MEDIUM) {
    // Should be a value of 24 on standard Linux platforms. Arbitrary value between
    // LOW and HIGH priority
    params.sched_priority = 
      (sched_get_priority_min(SCHED_FIFO) + sched_get_priority_max(SCHED_FIFO)) / 4 - 1;
  } else {
